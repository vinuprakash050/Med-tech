...
class OpenAIProvider(BaseLLMProvider):
    ...
    async def generate_response_with_system(self, system_prompt: str, user_prompt: str) -> str:
        ...
        logger.info(
            "LLM API call start provider=openai model=%s endpoint=%s",
            self.model,
            endpoint,
        )
        logger.info(
            "LLM API call waiting provider=openai model=%s",
            self.model,
        )
        ...
        logger.info(
            "LLM API call completed provider=openai model=%s status=%s",
            self.model,
            response.status_code,
        )

        if response.status_code >= 400:
            raise LLMProviderError(f"OpenAI request failed: {response.text}")

        data = response.json()
        try:
            return data["choices"][0]["message"]["content"].strip()
        except (KeyError, IndexError, TypeError) as exc:
            raise LLMProviderError("OpenAI response format was unexpected") from exc
